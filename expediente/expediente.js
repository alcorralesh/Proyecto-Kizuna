...
const renderAdminMauEditor=()=>{
  ...
  target.querySelectorAll('[data-mau-path]').forEach(input=>input.oninput=()=>{let value=input.type==='checkbox'?input.checked:Number(input.value);if(input.dataset.scale)value/=Number(input.dataset.scale);if(input.dataset.multiplier)value*=Number(input.dataset.multiplier);adminMauSet(config,input.dataset.mauPath,value);if(input.dataset.mauPath==='enabled')input.closest('label').querySelector('span').textContent=value?'Mau activado':'Mau desactivado';updateOutput(input.dataset.mauPath,value);validate()});

...
const renderAdminEditor=(profile,state,initialTab='summary')=>{
  ...
  document.querySelector('#admin-preview-alt00-response').onclick=()=>{if(!window.KizunaFinale?.previewResponseOutcome){alert('No se ha podido cargar la nueva vista previa.');return}window.KizunaFinale.previewResponseOutcome({assetBase:'../',displayName:profile.display_name||profile.email})};

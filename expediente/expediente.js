...
const privateHeader=document.querySelector('.dashboard>header'),privateHeaderTools=document.createElement('div'),privateMenuToggle=document.createElement('button');privateHeaderTools.className='private-header-tools';privateMenuToggle.id='private-menu-toggle';privateMenuToggle.type='button';privateMenuToggle.innerHTML='<span aria-hidden="true">••</span><b>Menú</b>';privateMenuToggle.setAttribute('aria-label','Abrir menú del expediente');privateMenuToggle.setAttribute('aria-expanded','false');if(privateHeader&&headerActions){privateHeader.insertBefore(privateHeaderTools,headerActions);privateHeaderTools.append(mailboxButton,privateMenuToggle,headerActions)}const closePrivateMenu=()=>{privateHeader?.classList.remove('private-menu-open');privateMenuToggle.setAttribute('aria-expanded','false');privateMenuToggle.setAttribute('aria-label','Abrir menú del expediente')};privateMenuToggle.onclick=event=>{event.stopPropagation();const open=!privateHeader.classList.contains('private-menu-open');privateHeader.classList.toggle('private-menu-open',open);privateMenuToggle.setAttribute('aria-expanded',String(open));privateMenuToggle.setAttribute('aria-label',open?'Cerrar menú del expediente':'Abrir menú del expediente')};headerActions?.addEventListener('click',event=>{if(event.target.closest('button'))closePrivateMenu()});document.addEventListener('click',event=>{if(privateHeader?.classList.contains('private-menu-open')&&!privateHeader.contains(event.target))closePrivateMenu()});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&privateHeader?.classList.contains('private-menu-open')){closePrivateMenu();privateMenuToggle.focus()}});
...
mailboxButton.onclick=()=>{closePrivateMenu();const opening=mailbox.style.display==='none';if(opening){renderMailbox();mailbox.style.display='block';patchState({mailRead:mailboxMessages().length}).finally(()=>{renderMailbox();updateMailboxIndicator()})}else mailbox.style.display='none'};
...
const mobileCaseHeaderStyle=document.createElement('style');mobileCaseHeaderStyle.textContent=`.private-header-tools{display:flex;align-items:center;gap:15px}.private-header-tools>#mailbox-toggle{position:relative;display:inline-flex;align-items:center;gap:6px}.private-header-tools>#mailbox-toggle i{display:grid;place-items:center;min-width:17px;height:17px;padding:0 4px;border-radius:9px;background:#7e1b19;color:#fff;font:7px/1 var(--mono);font-style:normal}.private-header-tools>#mailbox-toggle i[hidden]{display:none}#private-menu-toggle{display:none}@media(max-width:950px){.dashboard{overflow-x:hidden}.dashboard>header{min-height:60px;padding:max(7px,env(safe-area-inset-top)) 12px 7px!important;gap:10px;border-bottom:1px solid #c4b79f}.dashboard>header .access-brand{min-width:0;gap:8px}.dashboard>header .access-brand img{width:38px;height:38px}.dashboard>header .access-brand span{overflow:hidden;font-size:11px;letter-spacing:.13em;text-overflow:ellipsis;white-space:nowrap}.dashboard>header .access-brand small{display:block;font-size:5px}.dashboard>header .private-header-tools{position:relative;flex:0 0 auto;margin-left:auto;gap:6px}.dashboard>header .private-header-tools>#mailbox-toggle,.dashboard>header #private-menu-toggle{display:grid;grid-template-rows:15px 9px;place-items:center;width:48px;min-width:48px;height:44px;margin:0;padding:5px;border:1px solid #9b2d29;background:#f6f0e2;color:#7e1b19;box-shadow:3px 3px 0 #7e1b1914;cursor:pointer}.dashboard>header .private-header-tools>#mailbox-toggle>span,.dashboard>header #private-menu-toggle span{font:700 16px/1 var(--mono);letter-spacing:1px}.dashboard>header .private-header-tools>#mailbox-toggle>b,.dashboard>header #private-menu-toggle b{font:500 6px/1 var(--mono);letter-spacing:.08em;text-transform:uppercase}.dashboard>header #private-menu-toggle span{letter-spacing:2px}.dashboard>header.private-menu-open #private-menu-toggle{background:#7e1b19;color:#fff}.dashboard>header .private-header-tools>#mailbox-toggle.has-unread{border-color:#7e1b19;background:#7e1b19;color:#fff;box-shadow:0 0 0 3px #7e1b1924}.dashboard>header .private-header-tools>#mailbox-toggle i{position:absolute;right:-4px;top:-5px;border:2px solid #f6f0e2;background:#202726}.dashboard>header .header-actions{position:absolute;z-index:50;right:0;top:calc(100% + 7px);display:none!important;grid-template-columns:repeat(3,minmax(0,1fr));width:min(280px,calc(100vw - 24px));margin:0!important;padding:8px;gap:7px!important;border:1px solid #a89b83;background:#f6f0e2;box-shadow:9px 11px 0 #141b1840}.dashboard>header.private-menu-open .header-actions{display:grid!important}.dashboard>header .header-actions .sync-clock{display:none!important}.dashboard>header .header-actions button{position:relative;display:grid!important;grid-template-rows:27px 10px;place-items:center;width:100%!important;min-width:0!important;min-height:54px!important;margin:0!important;padding:6px!important;border:1px solid #c8bba3!important;border-radius:0!important;background:#eee4d2!important;color:#26312d!important;box-shadow:none!important;text-align:center}.dashboard>header .header-actions button span{display:grid;place-items:center;width:26px;height:26px;color:#7e1b19;font:18px/1 var(--serif)}.dashboard>header .header-actions button b{display:block!important;font:500 7px/1 var(--mono)!important;letter-spacing:.07em;text-transform:uppercase}.dashboard>header #sound-toggle.active{border-color:#9b2d29!important;background:#e3d2b7!important}.dashboard>header #exit{border-color:#9b2d29!important;color:#7e1b19!important}.dashboard>header .header-actions button:active{transform:translateY(1px)}.case-head{top:59px!important}}@media(max-width:390px){.dashboard>header .access-brand small{display:none}.dashboard>header .access-brand span{font-size:10px}.dashboard>header .private-header-tools{gap:4px}.dashboard>header .private-header-tools>#mailbox-toggle,.dashboard>header #private-menu-toggle{width:45px;min-width:45px}.dashboard>header .header-actions{width:min(270px,calc(100vw - 16px))}}`;document.head.appendChild(mobileCaseHeaderStyle);

...
if(!window.kizunaStorage){const storageAssetsScript=document.createElement('script');storageAssetsScript.src='../storage-assets.js?v=20260726-assets03';document.head.appendChild(storageAssetsScript)}
...
const KizunaSound=(()=>{
  ...
  const preferencesStorageKey='kizuna_sound_preferences_v1';
  const defaultPreferences=()=>({enabled:false,volume:.8,decided:false});
  const readPreferences=()=>{try{const stored=JSON.parse(localStorage.getItem(preferencesStorageKey)||'null');if(!stored||stored.decided!==true)return defaultPreferences();return{enabled:stored.enabled===true,volume:clamp(stored.volume??.8),decided:true}}catch{return defaultPreferences()}};
  const persistPreferences=()=>{try{localStorage.setItem(preferencesStorageKey,JSON.stringify(preferences))}catch{}};
  ...
  const setPreferences=changes=>{preferences={...preferences,...changes,volume:clamp(changes.volume??preferences.volume),decided:true};persistPreferences();syncToggle();return{...preferences}};
  ...
  const bindInterface=()=>{const actions=document.querySelector('#dashboard .header-actions');if(actions&&!document.querySelector('#sound-toggle')){const button=document.createElement('button');button.id='sound-toggle';button.type='button';button.innerHTML='<span class="kizuna-header-symbol" aria-hidden="true"><i class="kizuna-header-icon kizuna-header-icon-sound"></i></span><b>Sonido</b>';actions.prepend(button)}const button=document.querySelector('#sound-toggle');if(button&&!button.dataset.soundBound){button.dataset.soundBound='true';button.onclick=()=>{const enabled=!preferences.enabled;setPreferences({enabled});if(enabled)play('access_authorized')}}syncToggle()};
  ...
  window.addEventListener('storage',event=>{if(event.key!==preferencesStorageKey)return;preferences=readPreferences();syncToggle();if(preferences.decided)document.querySelector('#sound-consent')?.remove()});
...
const showEarlyAccessSequence=({preview=false,onComplete=()=>{}}={})=>{
  ...
  const reveal=()=>{
    ...
    scene.querySelector('.early-access-copy').scrollTop=0;
...
const recipientAccessErrorMessage=error=>{
  ...
  if(normalized.includes('cuenta existe')||normalized.includes('acceso ha sido desactivado'))return message;
  if(normalized.includes('email not confirmed'))return 'La cuenta existe, pero el correo todavía no está confirmado.';
  if(normalized.includes('banned'))return 'El acceso de este destinatario está desactivado en Administración.';
  return 'No se han podido verificar las credenciales de acceso.';
};
const loadRemoteProgress=async user=>{
  ...
  const client=await getSupabase();
  const [{data:loadedProgressRow,error},{data:profile,error:profileError}]=await Promise.all([
  ...
  let progressRow=loadedProgressRow;
  if(error)throw error;
  ...
  if(!progressRow){
    const initialState=emptyProgressState();
    const {data:createdProgress,error:createProgressError}=await client
      .from('expedient_progress')
      .insert({user_id:user.id,state:initialState,updated_at:new Date().toISOString()})
      .select('state,updated_at')
      .single();
    if(createProgressError){
      await client.auth.signOut();
      currentUser=null;
      const policyHint=['42501','PGRST301'].includes(String(createProgressError.code||''))
        ?' Comprueba las políticas de supabase-progress-rls.sql.'
        :'';
      throw new Error(`La cuenta existe, pero no se pudo inicializar su expediente.${policyHint}`);
    }
    progressRow=createdProgress;
  }
...
setTimeout(()=>{
  ...
  openDashboard=loadedProgress=>{
    ...
    const username=currentUser?.email?.split('@')[0]||'destinatario';
...
const revealCollectionAdvance=(collection,firstPendingSelector='')=>{
  const queued=pendingCollectionAdvance?.collection===collection?pendingCollectionAdvance:null;
  if(queued)pendingCollectionAdvance=null;
  const selector=queued?.selector||firstPendingSelector;
  if(!selector)return;
  const pending={collection,selector};
...
const mailboxButton=document.createElement('button'),mailbox=document.createElement('aside'),publicReturnButton=document.createElement('button'),pushChannelButton=document.createElement('button');mailboxButton.id='mailbox-toggle';mailboxButton.type='button';mailboxButton.setAttribute('aria-label','Abrir buzón del expediente');mailbox.id='mailbox';mailbox.style.cssText='display:none;position:fixed;right:5vw;top:82px;z-index:30;width:min(450px,calc(100vw - 32px));max-height:72vh;overflow:auto;background:#f6f0e2;color:#202726;border:1px solid #8b887d;box-shadow:12px 14px 30px #0004;padding:22px';pushChannelButton.id='push-channel-toggle';pushChannelButton.type='button';pushChannelButton.dataset.pushState='checking';pushChannelButton.innerHTML='<span class="kizuna-header-symbol" aria-hidden="true"><i class="kizuna-header-icon kizuna-header-icon-channel"></i></span><b>Canal</b><i class="push-channel-state" aria-hidden="true"></i>';pushChannelButton.setAttribute('aria-label','Comprobar Canal Seguro');pushChannelButton.title='Canal Seguro · comprobando terminal';publicReturnButton.id='return-public-site';publicReturnButton.type='button';publicReturnButton.innerHTML='<span class="kizuna-header-symbol" aria-hidden="true"><i class="kizuna-header-icon kizuna-header-icon-web"></i></span><b>Web</b>';publicReturnButton.setAttribute('aria-label','Volver a la web pública');publicReturnButton.title='Volver a la web pública';publicReturnButton.onclick=()=>{location.href='../index.html'};const headerActions=document.querySelector('.header-actions'),exitButton=document.querySelector('#exit');if(exitButton){exitButton.innerHTML='<span class="kizuna-header-symbol" aria-hidden="true"><i class="kizuna-header-icon kizuna-header-icon-exit"></i></span><b>Salir</b>';exitButton.setAttribute('aria-label','Cerrar sesión');exitButton.title='Cerrar sesión'}headerActions?.prepend(pushChannelButton);headerActions?.prepend(mailboxButton);headerActions?.insertBefore(publicReturnButton,exitButton);dash.appendChild(mailbox);
const paintMailboxButton=unread=>{mailboxButton.innerHTML=`<span class="kizuna-header-symbol" aria-hidden="true"><i class="kizuna-header-icon kizuna-header-icon-mailbox"></i></span><b>Buzón</b><i ${unread?'':'hidden'}>${Math.min(99,unread)}</i>`;mailboxButton.classList.toggle('has-unread',unread>0);mailboxButton.setAttribute('aria-label',unread?`Abrir buzón · ${unread} mensaje${unread===1?'':'s'} nuevo${unread===1?'':'s'}`:'Abrir buzón del expediente');mailboxButton.title=mailboxButton.getAttribute('aria-label')};
...
const syncPrivatePushChannel=async()=>{const state=await window.KizunaPWA?.getPushStatus?.();if(!state){setTimeout(()=>syncPrivatePushChannel(),250);return}pushChannelButton.dataset.pushState=state.state;pushChannelButton.setAttribute('aria-label',`Canal de comunicaciones · ${state.label}`);pushChannelButton.title=`Canal Seguro · ${state.label}`};
pushChannelButton.onclick=async()=>{await connectRecipientPush();await window.KizunaPWA?.openPushProtocol?.();void syncPrivatePushChannel()};
document.addEventListener('kizuna:push-state',()=>syncPrivatePushChannel());
setTimeout(()=>syncPrivatePushChannel(),0);
const mailboxEscape=value=>String(value??'').replace(/[&<>'"]/g,character=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[character]));
const automaticMailboxMessages=()=>{
  const done=read(),state=getState(),items=[];
  for(let i=1;i<=14;i++){
    const id=`KTB-${String(i).padStart(3,'0')}`;
    if(!done.includes(id))continue;
    items.push({id,subject:`Lectura confirmada · ${id}`,body:`La consulta del documento ${id} ha sido registrada correctamente. La secuencia autorizada ha sido actualizada.`,order:i,source:'system'});
  }
  if(done.includes('KTB-003'))items.push({id:'AC-01',subject:'Archivo complementario localizado',body:'Durante la reconstrucción del expediente se ha localizado el Archivo Complementario AC-01. Su contenido permanece en proceso de clasificación.',order:3.2,source:'system'});
  if((state.comicReadPages||[]).length>=11)items.push({id:'AC-01-COMPLETE',subject:'Registro ilustrado reconstruido',body:'Las 11 páginas disponibles de AC-01 han sido recuperadas e incorporadas al expediente.',order:3.8,source:'system'});
  [
    ['KTB-006','AR-01','Documentación operativa','La documentación operativa ha quedado disponible para su consulta conforme al protocolo de custodia.',6.2],
    ['KTB-007','AR-02','Planificación de la expedición','El itinerario clasificado y el mapa de la aventura ya pueden consultarse.',7.2],
    ['KTB-008','AR-03','Registros geográficos','Se ha autorizado el acceso a los registros geográficos recuperados.',8.2],
    ['KTB-009','AR-04','Archivos culturales','Las guías culturales recuperadas ya están disponibles para su revisión.',9.2],
    ['KTB-010','AR-05','Archivos personales','Se ha desbloqueado una carpeta de archivos personales vinculada al expediente.',10.2],
    ['KTB-011','AR-06','Datos recuperados del dispositivo','Se ha autorizado la consulta de los datos recuperados del dispositivo asociado al expediente.',11.2]
  ].forEach(([trigger,id,title,body,order])=>{
    if(done.includes(trigger))items.push({id,subject:`Acceso autorizado · ${id}`,body:`${title}. ${body}`,order,source:'system'});
  });
  if(done.includes('AR01-BILLETES'))items.push({id:'AR01-BILLETES',subject:'Registros de transporte verificados',body:'Todos los billetes recuperados de AR-01 han sido confirmados y archivados.',order:6.5,source:'system'});
  if(ar03Complete())items.push({id:'AR03',subject:'Registros geográficos completados',body:'Las guías de ciudades y las entradas de templos han sido incorporadas al expediente.',order:8.5,source:'system'});
  const finalDetected=done.includes('KTB-014')&&(state.finalAlertShown||state.finalFileSeen||state.finalFlowStage==='interrupted'||done.includes('FINAL-01'));
  if(finalDetected){
    const incorporated=done.includes('FINAL-01');
    items.push({
      id:'FINAL-01',
      subject:incorporated?'Archivo no catalogado incorporado':'ATENCIÓN · Archivo no catalogado localizado',
      body:incorporated?'FINAL-01 ha sido examinado e incorporado al cierre del expediente. El registro permanece disponible para futuras consultas.':'La verificación final ha localizado un archivo no catalogado. Su examen es obligatorio para completar el cierre.',
      order:100,
      urgent:!incorporated,
      action:'final-file',
      source:'system'
  ...
  if(finalFlowClosed()){
    items.push({id:'KTB-014-FINAL',subject:'Expediente archivado',body:'La consulta ha finalizado. El expediente PROJECT JAPAN ha sido archivado con integridad documental preservada.',order:14.2,source:'system'});
    items.push({id:'ALBERTO',subject:'Comunicación personal disponible',body:'Alberto ha dejado una comunicación fuera del Archivo Central. Puedes consultarla desde la web pública de KIZUNA.',order:14.4,action:'alberto',source:'system'});
  }
  if(state.alt00Discovered)items.push({id:'ALT-00',subject:'Señal residual detectada · ALT-00',body:'El Protocolo Omega ha liberado una línea temporal descartada vinculada a la decisión final.',order:101,urgent:true,source:'system'});
  if(state.alt00Completed)items.push({id:'ALT-00-COMPLETE',subject:'Registro alternativo completado',body:'Las 10 páginas de ALT-00 han sido consultadas. La línea temporal descartada queda preservada en el expediente.',order:102,source:'system'});
...
const mobileCaseHeaderStyle=document.createElement('style');mobileCaseHeaderStyle.textContent=`.private-header-tools{display:flex;align-items:center;gap:15px}.private-header-tools>#mailbox-toggle{position:relative;display:inline-flex;align-items:center;gap:6px}.private-header-tools>#mailbox-toggle i{display:grid;place-items:center;min-width:17px;height:17px;padding:0 4px;border-radius:9px;background:#7e1b19;color:#fff;font:7px/1 var(--mono);font-style:normal}.private-header-tools>#mailbox-toggle i[hidden]{display:none}@media (max-width:600px), (orientation:portrait) and (min-width:601px) and (max-width:1100px), (orientation:landscape) and (max-width:1000px) and (max-height:520px), (orientation:landscape) and (min-width:901px) and (max-width:1400px) and (min-height:600px) and (pointer:coarse){.dashboard{overflow-x:hidden;padding-top:max(58px,calc(48px + env(safe-area-inset-top)))}.dashboard>header{position:fixed!important;inset:0 0 auto 0;width:100%;min-height:58px;padding:max(7px,env(safe-area-inset-top)) 12px 7px!important;gap:8px;border-bottom:1px solid #c4b79f}.dashboard>header .access-brand{min-width:0;gap:7px}.dashboard>header .access-brand img{width:36px;height:36px}.dashboard>header .access-brand span{overflow:hidden;font-size:10px;letter-spacing:.12em;text-overflow:ellipsis;white-space:nowrap}.dashboard>header .access-brand small{display:block;font-size:5px}.dashboard>header .private-header-tools{flex:0 0 auto;margin-left:auto;gap:5px}.dashboard>header .header-actions{display:flex!important;align-items:center;gap:5px!important;margin:0!important}.dashboard>header .header-actions .sync-clock{display:none!important}.dashboard>header .private-header-tools>#mailbox-toggle,.dashboard>header .header-actions button{position:relative;display:grid!important;place-items:center;width:40px!important;min-width:40px!important;height:40px!important;min-height:40px!important;margin:0!important;padding:0!important;border:1px solid #a88479!important;border-radius:0!important;background:#f6f0e2!important;color:#7e1b19!important;box-shadow:2px 2px 0 #7e1b1912!important}.dashboard>header .private-header-tools button>span{display:grid;place-items:center;width:100%;height:100%;font:18px/1 var(--serif);color:inherit}.dashboard>header .private-header-tools button>b{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0);clip-path:inset(50%);white-space:nowrap}.dashboard>header .private-header-tools>#mailbox-toggle.has-unread{border-color:#7e1b19!important;background:#7e1b19!important;color:#fff!important;box-shadow:0 0 0 3px #7e1b1920!important}.dashboard>header .private-header-tools>#mailbox-toggle i{position:absolute;right:-5px;top:-5px;border:2px solid #f6f0e2;background:#202726}.dashboard>header #sound-toggle.active{border-color:#7e1b19!important;background:#e3d2b7!important}.dashboard>header #exit{border-color:#7e1b19!important}.dashboard>header .private-header-tools button:active{transform:translateY(1px)}.case-head{position:relative!important;top:auto!important;min-height:0!important;padding:10px 7vw 12px!important}}@media(max-width:390px){.dashboard{padding-top:max(52px,calc(43px + env(safe-area-inset-top)))}.dashboard>header{min-height:52px;padding-left:8px!important;padding-right:8px!important}.dashboard>header .access-brand{gap:5px}.dashboard>header .access-brand img{width:32px;height:32px}.dashboard>header .access-brand span{font-size:9px}.dashboard>header .access-brand small{display:none}.dashboard>header .private-header-tools,.dashboard>header .header-actions{gap:3px!important}.dashboard>header .private-header-tools>#mailbox-toggle,.dashboard>header .header-actions button{width:35px!important;min-width:35px!important;height:35px!important;min-height:35px!important}.dashboard>header .private-header-tools button>span{font-size:16px}.case-head{padding:8px 6vw 10px!important}}`;document.head.appendChild(mobileCaseHeaderStyle);
...
function revealComicCard(card,{scroll=true}={}){
  ...
  clearAcRevealTimers();
  const reduced=matchMedia('(prefers-reduced-motion:reduce)').matches;
  const compact=matchMedia('(max-width:750px)').matches;
  let finished=false;
  const finish=()=>{
    if(finished||!card.isConnected)return;
    finished=true;
    clearAcRevealTimers();
    card.classList.add('is-ac-revealed');
    card.querySelector('.comic-discovery-skip')?.setAttribute('disabled','');
    acRevealTimers.push(setTimeout(()=>{
      if(!card.isConnected)return;
      card.classList.remove('is-ac-reveal','is-ac-scanning','is-ac-opening','is-ac-revealed');
      card.querySelector('.comic-discovery-layer')?.remove();
      card.querySelector('.comic-reveal-stamp')?.remove();
    },reduced?20:620));
  };
...
// acción y mantiene el clic táctil y de ratón en el mismo flujo.
document.querySelector('#documents').addEventListener('click',event=>{
  ...
  if(!button||button.disabled||button.hasAttribute('data-final-resume'))return;
  event.preventDefault();
...
const nextRequiredDocument=(id,buttonId,locked=false)=>{const confirmed=read().includes(id),available=!locked&&!confirmed;return `<aside class="folder-next-phase ${confirmed?'is-complete':locked?'is-locked':''}"><div class="folder-next-symbol" aria-hidden="true">${confirmed?'✓':locked?'⌁':'↗'}</div><div class="folder-next-copy"><p>DOCUMENTO DE CONTINUIDAD</p><h4><span>${id}</span> Actualización del expediente</h4><small>${confirmed?'La lectura de este documento ya está confirmada. Puedes volver a consultarlo.':locked?'Completa la secuencia documental de esta carpeta para retirar el sello.':'La carpeta está completa. Consulta y confirma este documento para continuar.'}</small></div><div class="folder-next-state"><span>${confirmed?'ARCHIVADO':available?'DISPONIBLE':'SELLADO'}</span><strong>${confirmed?'LECTURA CONFIRMADA':available?'LECTURA PENDIENTE':'ACCESO BLOQUEADO'}</strong></div><button id="${buttonId}" type="button" ${locked?'disabled':''}>${confirmed?'Revisar':available?'Consultar':'Bloqueado'} ${id} <span>${locked?'⌕':'→'}</span></button></aside>`};
...
function openTicketMosaic(){
  ...
  document.querySelectorAll('#doc-body .ticket-card img').forEach(image=>{image.dataset.storageLoading='eager';image.dataset.storagePriority='high';image.loading='eager';image.fetchPriority='high'});
...
function openAr03(){
  ...
  document.querySelector('#doc-body').innerHTML=`<section class="folder-index folder-sequence-index ar03-folder-index is-short" data-file-count="2"><header><div><p class="system-line">ÍNDICE DE LA CARPETA</p><h3>Registros geográficos</h3></div><p><strong>${reviewed}</strong> de ${total}<span>registros consultados</span></p></header><p class="folder-intro">Selecciona una subcarpeta para consultar sus registros recuperados.</p><p class="folder-sequence-label"><span>SECUENCIA DOCUMENTAL</span><strong>${ready?'Carpeta completa':'2 lotes documentales'}</strong></p><div class="folder-file-list"><button id="ar03-cities" class="folder-file ${citiesBatch?'is-read':'is-ready'}"><span class="folder-file-number">01</span><span class="folder-file-copy"><strong>Guías de ciudades</strong><small>${citiesBatch?'LOTE CONFIRMADO':`${citiesDone} DE ${ar03Cities.length} REGISTROS CONSULTADOS`}</small></span><span class="folder-file-action">${citiesBatch?'REVISAR':'ABRIR'}</span></button><button id="ar03-temples" class="folder-file ${templesBatch?'is-read':'is-ready'}"><span class="folder-file-number">02</span><span class="folder-file-copy"><strong>Entradas de templos</strong><small>${templesBatch?'LOTE CONFIRMADO':`${letterDone?'CARTA CONSULTADA · ':''}${templesDone} DE ${ar03Temples.length} REGISTROS CONSULTADOS`}</small></span><span class="folder-file-action">${templesBatch?'REVISAR':'ABRIR'}</span></button></div><footer>${nextRequiredDocument('KTB-009','ar03-update',!ready)}</footer></section>`;
  if(!viewer.open)viewer.showModal();document.querySelector('#ar03-cities').onclick=()=>openAr03Mosaic('cities');document.querySelector('#ar03-temples').onclick=()=>{if(!read().includes('AR03-CARTA'))openAr03Item('AR03-CARTA','Carta de presentación','../assets/documents/AR-03/Templos/000-Carta.png');else openAr03Mosaic('temples')};const update=document.querySelector('#ar03-update');if(update&&!update.disabled)update.onclick=()=>openDoc('KTB-009');
}
...
const showRecoveredDocumentReady=id=>{
  // El microevento se decide una sola vez al terminar la recuperación de un
  // documento no leído. No observa ni reacciona a cambios posteriores del DOM.
  if(!read().includes(id)&&!microeventRecoveryPassed.has(id)){
    if(microeventRecoveryRunning.has(id))return;
    microeventRecoveryRunning.add(id);
    void (async()=>{
      try{
        const module=await getMicroevents();
        if(module)await Promise.race([
          module.runRecovery(id,{container:document.querySelector('#doc-body')}),
          new Promise(resolve=>setTimeout(()=>resolve(true),9000))
        ]);
      }catch(error){
        console.warn('Microevento de recuperación omitido de forma segura.',error);
      }finally{
        microeventRecoveryPassed.add(id);
        microeventRecoveryRunning.delete(id);
        if(viewer.open&&active===id)showRecoveredDocumentReady(id);
      }
    })();
    return;
  }
  document.querySelector('#doc-title').textContent='Archivo recuperado';
  document.querySelector('#doc-body').innerHTML=`<section class="recovery-ready"><div class="recovery-ready-symbol">◇</div><p class="recovery-kicker">ARCHIVO RECUPERADO</p><h3>${id}</h3><p class="recovery-ready-name">Documento recuperado</p><dl><div><dt>INTEGRIDAD DOCUMENTAL</dt><dd>100 %</dd></div><div><dt>ESTADO</dt><dd>Disponible para consulta</dd></div></dl><div class="recovery-final-stamp">ARCHIVO RECUPERADO</div><button type="button" id="open-recovered-document">Abrir documento →</button></section>`;
  document.querySelector('#open-recovered-document').onclick=()=>{clearRecoveryTimers();document.querySelector('.reader-content').classList.remove('is-recovery');showDoc(id)};
};
...
function openAcInfo(){
  ...
  next.style.display='none';
  mark.style.display='none';
  const pages=Array.from({length:11},(_,i)=>`KTB-${String(i+4).padStart(3,'0')}`).filter(id=>read().includes(id)).length;
  const pageBar=Array.from({length:11},(_,index)=>`<i class="${index<pages?'is-recovered':''}" aria-hidden="true"><span>${String(index+1).padStart(2,'0')}</span></i>`).join('');
  const paper=document.querySelector('.paper'),body=document.querySelector('#doc-body');
  paper.classList.add('is-ac-info-paper');
  body.classList.add('is-ac-info');
  document.querySelector('#doc-type').textContent='DIVISIÓN DE ARCHIVOS TEMPORALES · REGISTRO ILUSTRADO';
  document.querySelector('#doc-title').textContent='ARCHIVO COMPLEMENTARIO AC-01';
  body.innerHTML=`<section class="ac-record-info"><header><div><p>ARCHIVO COMPLEMENTARIO · AC-01</p><h3>Registro ilustrado<br><em>recuperado.</em></h3></div><span>ORIGEN<br><strong>NO CATALOGADO</strong></span></header><dl class="ac-info-metadata"><div><dt>CLASIFICACIÓN</dt><dd>No determinada</dd></div><div><dt>INTEGRIDAD</dt><dd class="is-valid">100 %</dd></div><div><dt>ESTADO</dt><dd>${pages?'Reconstrucción parcial':'En reconstrucción'}</dd></div><div><dt>PÁGINAS DISPONIBLES</dt><dd>${pages} / 11</dd></div></dl><section class="ac-info-progress"><div><p>PÁGINAS RECUPERADAS</p><strong>${pages} / 11</strong></div><div class="comic-page-bar" role="img" aria-label="${pages} de 11 páginas recuperadas">${pageBar}</div></section><aside class="ac-info-observation"><span>OBSERVACIÓN · ARCHIVO CENTRAL</span><p>Durante la reconstrucción del expediente se ha localizado un documento ilustrado que no figura en el inventario original.</p><p>Su contenido parece guardar relación con el expediente, aunque su procedencia no ha podido verificarse.</p></aside>${pages?`<footer class="ac-info-action"><div><span>REGISTRO DISPONIBLE</span><strong>${pages} ${pages===1?'página recuperada':'páginas recuperadas'}</strong></div><button id="ac-open-record" type="button">Abrir registro <b>→</b></button></footer>`:'<footer class="ac-info-awaiting"><span>01</span><div><p>PRÓXIMA RECUPERACIÓN</p><strong>KTB-004 incorporará la primera página</strong><small>Continúa la secuencia principal para iniciar la reconstrucción.</small></div></footer>'}</section>`;
  if(!viewer.open)viewer.showModal();
...
document.querySelector('#gate-consent').onchange=event=>document.querySelector('#gate-continue').disabled=!event.target.checked;
document.querySelector('#gate-continue').onclick=()=>{patchState({legalAccepted:true});gate.hidden=true;dash.hidden=false;render()};
document.querySelector('#exit').onclick=()=>{location.href='../index.html'};
readerCloseButton.onclick=async()=>{if(viewer.classList.contains('is-reader-fullscreen')||document.fullscreenElement===viewer)await exitReaderFullscreen();else closeViewerAndFocusPending()};
mark.onclick=async()=>{if(active==='FINAL-01'){if(finalFileRead())return;mark.disabled=true;mark.textContent='Registrando lectura…';try{await showFinalPublicTransition()}catch(error){console.error('No se pudo registrar la lectura de FINAL-01.',error);mark.disabled=false;mark.textContent='Confirmar lectura'}return}const done=read();if(!done.includes(active)){const nextRead=[...done,active];await(active==='KTB-014'?withKtb14ReadMutation(()=>save(nextRead)):save(nextRead))}if(active.startsWith('AR03-')){if(ar03Complete())openAr03();else if(active==='AR03-CARTA')openAr03Mosaic('temples');else openAr03Mosaic(active.startsWith('AR03-C-')?'cities':'temples');return}const parent=fileFolder(active);if(parent){openFolder(parent);return}if(active.startsWith('KTB-')){const id=active;syncKtb(id,()=>{if(id==='KTB-014')startFinale();else closeViewerAndFocusPending({confirmedId:id})});return}closeViewerAndFocusPending()};
next.onclick=()=>{const index=sequence.indexOf(active);if(index<sequence.length-1&&allowed(sequence[index+1]))openDoc(sequence[index+1])};
...
const adminNavigationHint=document.createElement('p');
adminNavigationHint.className='admin-nav-hint';
adminNavigationHint.setAttribute('aria-hidden','true');
adminNavigationHint.innerHTML='Desliza para cambiar de sección <span>↔</span>';
document.querySelector('.admin-rail .admin-brand').after(adminNavigationHint);
const adminExitButton=document.querySelector('#admin-exit');
adminExitButton.type='button';
...
adminMauNav.type='button';adminMauNav.dataset.adminView='mau';adminMauNav.innerHTML='<span>10</span>Mau';
document.querySelector('.admin-sidebar').appendChild(adminMauNav);
const adminMauView=document.createElement('section');
adminMauView.id='admin-mau-view';adminMauView.className='admin-view';adminMauView.hidden=true;
adminMauView.innerHTML=`<p class="system-line">MASCOTA KIZUNA · CONFIGURACIÓN ESTÁTICA</p><h1>Administración de<br><em>Mau.</em></h1><p class="admin-intro">Ajusta su comportamiento global sin utilizar Supabase. Descarga el archivo resultante y publícalo con el proyecto para aplicarlo a todos los visitantes.</p><aside class="admin-mau-file-note"><strong>SIN DATOS DE USUARIO</strong><span>Esta sección no consulta ni modifica expedientes. Los cambios sólo se aplican cuando se sustituye y publica <code>mascot/mau-config.js</code>.</span></aside><section class="admin-mau-guide" aria-labelledby="admin-mau-guide-title"><header><p class="system-line">CÓMO APLICAR LOS CAMBIOS</p><h2 id="admin-mau-guide-title">Cuatro pasos para actualizar a Mau</h2></header><ol><li><span>01</span><div><strong>Ajusta su comportamiento</strong><p>Cambia las frecuencias, tiempos y frases que quieras. Puedes probar posturas y mensajes en la vista previa.</p></div></li><li><span>02</span><div><strong>Revisa los avisos</strong><p>El botón de descarga sólo se habilitará cuando la configuración sea válida.</p></div></li><li><span>03</span><div><strong>Descarga la configuración</strong><p>Pulsa <b>Descargar configuración</b>. Se guardará un archivo llamado <code>mau-config.js</code>.</p></div></li><li><span>04</span><div><strong>Envíame el archivo para publicarlo</strong><p>Adjunta el archivo descargado en Codex y escribe: “aplica y publica esta configuración de Mau”. Yo sustituiré el archivo correcto y publicaré la web.</p></div></li></ol><footer><strong>IMPORTANTE</strong><span>Descargar el archivo no cambia la web. Mau se actualizará para todos únicamente después de publicarlo.</span></footer></section><div id="admin-mau-editor"><p>Cargando configuración de Mau…</p></div>`;
document.querySelector('.admin-views').appendChild(adminMauView);

const adminViews={users:document.querySelector('#admin-users-view'),mailbox:document.querySelector('#admin-mailbox-view'),media:document.querySelector('#admin-media-view'),blog:document.querySelector('#admin-blog-view'),events:adminEventsView,shop:adminShopView,communications:adminCommunicationsView,sound:adminSoundView,directMessages:adminDirectMessagesView,mau:adminMauView};
const adminViewTitles={users:['USUARIOS','Gestión de expedientes'],mailbox:['BUZÓN','Mensajes recibidos'],media:['MEDIA','Biblioteca de imágenes'],blog:['BLOG','Gestión de artículos'],events:['EVENTOS','Gestión de eventos'],shop:['TIENDA','Catálogo simulado'],communications:['COMUNICACIONES','Avisos de actividad'],sound:['SONIDO','Ambientación del expediente'],directMessages:['MENSAJES','Comunicación directa'],mau:['MAU','Configuración de la mascota']};
...
document.querySelectorAll('.admin-sidebar button').forEach(button=>button.onclick=()=>{
  ...
  if(matchMedia('(max-width:750px)').matches){const nav=button.parentElement;nav.scrollTo({left:button.offsetLeft-(nav.clientWidth-button.offsetWidth)/2,behavior:matchMedia('(prefers-reduced-motion:reduce)').matches?'auto':'smooth'})}
  ...
  if(button.dataset.adminView==='mau')loadAdminMauConfig();
});

let adminMauPublishedConfig=null,adminMauDraft=null;
const cloneAdminMauConfig=value=>JSON.parse(JSON.stringify(value));
const adminMauGet=(source,path)=>path.split('.').reduce((value,key)=>value?.[key],source);
const adminMauSet=(source,path,value)=>{const keys=path.split('.'),last=keys.pop();const target=keys.reduce((item,key)=>item[key],source);target[last]=value};
const adminMauLines=value=>(Array.isArray(value)?value:[]).join('\n');
const adminMauTimingLabels=Object.freeze({peek:'Espera inicial al asomarse',transition:'Transición a postura normal',message:'Duración del mensaje',leave:'Duración de salida',footerPause:'Pausa antes de dormir',sleep:'Tiempo dormido',sleepFade:'Desvanecimiento al despertar',interactionHold:'Respuesta al tocarlo',blinkMin:'Parpadeo mínimo',blinkMax:'Parpadeo máximo',blinkFrame:'Velocidad del parpadeo',reactionFrame:'Velocidad de reacción',waveFrame:'Velocidad del saludo',wakeFrame:'Velocidad al despertar',wakeHold:'Pausa al despertar',albertoDelayMin:'Gesto especial · espera mínima',albertoDelayMax:'Gesto especial · espera máxima',albertoFrame:'Velocidad del gesto especial',albertoHold:'Pausa del gesto especial'});
const adminMauRecommendedTimings=Object.freeze({peek:1700,transition:260,message:7600,leave:1100,footerPause:900,sleep:8200,sleepFade:900,interactionHold:4400,blinkMin:3200,blinkMax:6200,blinkFrame:135,reactionFrame:165,waveFrame:180,wakeFrame:320,wakeHold:620,albertoDelayMin:1600,albertoDelayMax:2800,albertoFrame:420,albertoHold:700});
const adminMauTimingValue=value=>value>=1000?`${String(Number((value/1000).toFixed(1))).replace('.',',')} s (${value} ms)`:`${value} ms`;
const adminMauFlowLabels=Object.freeze({messagePrompt:'Aviso inicial de la carta',physicalPrompt:'Buscar el sobre físico',responseReminder:'Respuesta pendiente',decision:'Elección final',deferredNotice:'Carta pospuesta',letterClosed:'Carta cerrada sin responder',farewell:'Despedida final'});
const adminMauSerialize=config=>`const deepFreeze = value => {\n  if (!value || typeof value !== 'object' || Object.isFrozen(value)) return value;\n  Object.values(value).forEach(deepFreeze);\n  return Object.freeze(value);\n};\n\nconst MAU_CONFIG = deepFreeze(${JSON.stringify(config,null,2)});\n\nglobalThis.KIZUNA_MAU_CONFIG = MAU_CONFIG;\n`;
const renderAdminMauEditor=()=>{
  const target=document.querySelector('#admin-mau-editor');if(!target||!adminMauDraft)return;
  const config=adminMauDraft;
  const automaticGroups=[['dialogue.general','Frases habituales'],['dialogue.residual','Frases de anomalía temporal'],['dialogue.alberto','Frases sobre Alberto']];
  const interactionGroups=[['interaction.awake','Al tocar a Mau despierto'],['interaction.sleeping','Al tocar a Mau dormido']];
  const requiredLineGroups=[...automaticGroups,...interactionGroups,['mapFlow','Mensajes del mapa · 11 destinos']];
  target.innerHTML=`<div class="admin-mau-layout"><section class="admin-mau-controls"><header><div><p class="system-line">COMPORTAMIENTO GLOBAL</p><h2>Apariciones y frecuencia</h2></div><label class="admin-mau-enabled"><input type="checkbox" data-mau-path="enabled" ${config.enabled?'checked':''}><i></i><span>${config.enabled?'Mau activado':'Mau desactivado'}</span></label></header><div class="admin-mau-field-grid"><label>Primera aparición <output data-mau-output="triggerProgress">${Math.round(config.triggerProgress*100)} % de página</output><input type="range" min="5" max="90" step="5" value="${Math.round(config.triggerProgress*100)}" data-mau-path="triggerProgress" data-mau-scale="100"></label><label>Espera mínima <output data-mau-output="minimumPageAge">${(config.minimumPageAge/1000).toFixed(1)} s</output><input type="range" min="0" max="30" step="1" value="${config.minimumPageAge/1000}" data-mau-path="minimumPageAge" data-mau-scale="0.001"></label><label>Apariciones despierto<input type="number" min="1" max="5" step="1" value="${config.maxAwakeAppearances}" data-mau-path="maxAwakeAppearances"></label><label>Repetición mínima<input type="number" min="5" max="180" step="5" value="${config.repeatCooldown/1000}" data-mau-path="repeatCooldown" data-mau-multiplier="1000"><small>Segundos entre apariciones</small></label><label>Duración de la burbuja<input type="number" min="2" max="30" step=".5" value="${config.timings.message/1000}" data-mau-path="timings.message" data-mau-multiplier="1000"><small>Segundos visibles</small></label></div></section><aside class="admin-mau-preview"><p class="system-line">VISTA PREVIA</p><div class="admin-mau-preview-stage"><div class="admin-mau-preview-bubble" id="admin-mau-preview-message">${adminEditorEscape(config.dialogue.general[0])}</div><img id="admin-mau-preview-image" src="../mascot/assets/sprites/mau-guide.webp" alt="Previsualización de Mau"></div><div class="admin-mau-preview-actions"><button type="button" data-mau-pose="peek">Asomado</button><button type="button" data-mau-pose="guide">Normal</button><button type="button" data-mau-pose="leave">Marchándose</button><button type="button" data-mau-pose="sleep">Dormido</button><button type="button" id="admin-mau-preview-random">Probar frase automática</button></div></aside><section class="admin-mau-automatic admin-mau-numbered"><header class="admin-mau-block-header"><span class="admin-mau-step">01</span><div><p class="system-line">FRASES AUTOMÁTICAS</p><h2>Lo que Mau dice cuando aparece</h2></div></header><p class="admin-mau-section-intro">Primero se elige un grupo según estos porcentajes y después una frase al azar dentro de ese grupo. Añadir frases aporta variedad, pero no aumenta la probabilidad del grupo.</p><div class="admin-mau-field-grid"><label>Frases sobre Alberto <output data-mau-output="dialogue.albertoChance">${Math.round(config.dialogue.albertoChance*100)} %</output><input type="range" min="0" max="100" step="5" value="${Math.round(config.dialogue.albertoChance*100)}" data-mau-path="dialogue.albertoChance" data-mau-scale="100"></label><label>Frases de anomalía temporal <output data-mau-output="dialogue.residualChance">${Math.round(config.dialogue.residualChance*100)} %</output><input type="range" min="0" max="100" step="5" value="${Math.round(config.dialogue.residualChance*100)}" data-mau-path="dialogue.residualChance" data-mau-scale="100"></label><label>Contextuales dentro de las habituales <output data-mau-output="dialogue.contextualChance">${Math.round(config.dialogue.contextualChance*100)} %</output><input type="range" min="0" max="100" step="5" value="${Math.round(config.dialogue.contextualChance*100)}" data-mau-path="dialogue.contextualChance" data-mau-scale="100"></label></div><div class="admin-mau-probability-help"><p><strong>Alberto y anomalías</strong> usan los porcentajes indicados.</p><p><strong>Habituales</strong> reciben automáticamente el porcentaje que quede libre.</p><p><strong>Contextuales</strong> se eligen dentro de las habituales cuando existen frases para la sección visible.</p></div><p id="admin-mau-probability-summary"></p><div class="admin-mau-line-help"><strong>Cómo editar las listas</strong><span>Una línea equivale a una frase. Pulsa Intro para añadir otra o borra una línea para eliminarla. Conserva al menos una frase en cada una de estas tres listas.</span></div><div class="admin-mau-dialogue-grid">${automaticGroups.map(([path,title])=>`<label>${title}<textarea rows="7" data-mau-lines="${path}">${adminEditorEscape(adminMauLines(adminMauGet(config,path)))}</textarea></label>`).join('')}</div><details><summary>Frases contextuales por sección</summary><p>También son automáticas, pero sólo pueden salir dentro del porcentaje habitual. Una sección sin frases contextuales utilizará las habituales.</p><div class="admin-mau-dialogue-grid">${Object.entries(config.dialogue.contextual).map(([section,lines])=>`<label>${section}<textarea rows="5" data-mau-lines="dialogue.contextual.${section}">${adminEditorEscape(adminMauLines(lines))}</textarea></label>`).join('')}</div></details></section><section class="admin-mau-interactions admin-mau-numbered"><header class="admin-mau-block-header"><span class="admin-mau-step">02</span><div><p class="system-line">RESPUESTAS AL TOCARLO</p><h2>Lo que Mau dice al interactuar</h2></div></header><p class="admin-mau-section-intro">Estas frases no aparecen automáticamente y no utilizan los porcentajes anteriores. Sólo se elige una cuando Jose toca a Mau.</p><div class="admin-mau-line-help"><strong>Puedes añadir o quitar respuestas</strong><span>Escribe una por línea y conserva al menos una respuesta para Mau despierto y otra para Mau dormido.</span></div><div class="admin-mau-dialogue-grid">${interactionGroups.map(([path,title])=>`<label>${title}<textarea rows="7" data-mau-lines="${path}">${adminEditorEscape(adminMauLines(adminMauGet(config,path)))}</textarea></label>`).join('')}</div></section><section class="admin-mau-narrative admin-mau-numbered"><header class="admin-mau-block-header"><span class="admin-mau-step">03</span><div><p class="system-line">MENSAJES NARRATIVOS FIJOS</p><h2>Mapa y carta de Alberto</h2></div></header><p class="admin-mau-section-intro">No se eligen al azar ni usan probabilidades. Cada texto corresponde a un momento concreto de la historia.</p><div class="admin-mau-narrative-grid"><section><h3>Mapa del viaje</h3><p>Deben existir exactamente 11 líneas y conservar su orden: una por cada destino.</p><label>Mensajes del mapa · 11 destinos<textarea rows="12" data-mau-lines="mapFlow">${adminEditorEscape(adminMauLines(config.mapFlow))}</textarea></label></section><section><h3>Carta de Alberto</h3><p>Los siete momentos son fijos. Puedes cambiar el texto, pero no añadir ni eliminar pasos.</p><div class="admin-mau-flow-fields">${Object.entries(adminMauFlowLabels).map(([key,label])=>`<label>${label}<textarea rows="3" data-mau-text="albertoFlow.${key}">${adminEditorEscape(config.albertoFlow[key])}</textarea></label>`).join('')}</div></section></div></section><section class="admin-mau-secret admin-mau-numbered"><header class="admin-mau-block-header"><span class="admin-mau-step">04</span><div><p class="system-line">ANIMACIÓN ESPECIAL · ALBERTO</p><h2>Gesto independiente de Mau</h2></div><a href="../index.html?mau=alberto" target="_blank" rel="noopener">Probar con mau=alberto ↗</a></header><p class="admin-mau-section-intro">En una visita normal, este gesto sólo puede aparecer a partir de la segunda aparición de Mau. La prueba local lo fuerza para que puedas revisarlo sin esperar.</p><div class="admin-mau-field-grid"><label>Probabilidad de la animación secreta <output data-mau-output="specialGestureChance">${Math.round(config.specialGestureChance*100)} %</output><input type="range" min="0" max="100" step="5" value="${Math.round(config.specialGestureChance*100)}" data-mau-path="specialGestureChance" data-mau-scale="100"><small>Recomendado: 50 %. Se aplica cuando el gesto ya puede aparecer.</small></label><label>Espera mínima<input type="number" min="50" max="180000" step="50" value="${config.timings.albertoDelayMin}" data-mau-path="timings.albertoDelayMin"><small>Recomendado: ${adminMauTimingValue(adminMauRecommendedTimings.albertoDelayMin)}</small></label><label>Espera máxima<input type="number" min="50" max="180000" step="50" value="${config.timings.albertoDelayMax}" data-mau-path="timings.albertoDelayMax"><small>Recomendado: ${adminMauTimingValue(adminMauRecommendedTimings.albertoDelayMax)}</small></label><label>Velocidad de los fotogramas<input type="number" min="50" max="180000" step="50" value="${config.timings.albertoFrame}" data-mau-path="timings.albertoFrame"><small>Recomendado: ${adminMauTimingValue(adminMauRecommendedTimings.albertoFrame)}</small></label><label>Pausa al terminar el gesto<input type="number" min="50" max="180000" step="50" value="${config.timings.albertoHold}" data-mau-path="timings.albertoHold"><small>Recomendado: ${adminMauTimingValue(adminMauRecommendedTimings.albertoHold)}</small></label></div><label class="admin-mau-secret-message">Frase durante la animación<textarea rows="3" data-mau-text="dialogue.specialGestureMessage">${adminEditorEscape(config.dialogue.specialGestureMessage)}</textarea></label><p class="admin-mau-secret-note">La prueba <code>?mau=alberto</code> fuerza la aparición y utiliza una espera corta. La probabilidad y la espera aleatoria configuradas aquí se aplican durante las visitas normales.</p></section><details class="admin-mau-advanced"><summary>Tiempos y animaciones avanzadas</summary><p class="admin-mau-section-intro">Los valores están expresados en milisegundos. El recomendado conserva el ritmo original diseñado para Mau.</p><div class="admin-mau-timing-grid">${Object.entries(config.timings).filter(([key])=>key!=='message'&&!key.startsWith('alberto')).map(([key,value])=>`<label>${adminMauTimingLabels[key]||key}<input type="number" min="50" max="180000" step="50" value="${value}" data-mau-path="timings.${key}"><small>Recomendado: ${adminMauTimingValue(adminMauRecommendedTimings[key])}</small></label>`).join('')}</div></details><footer class="admin-mau-export"><div><p class="system-line">ARCHIVO DE CONFIGURACIÓN</p><strong>mascot/mau-config.js</strong><span id="admin-mau-status" role="status">Sin cambios.</span></div><button type="button" id="admin-mau-restore">Restaurar archivo publicado</button><button type="button" id="admin-mau-download">Descargar configuración</button></footer></div>`;
  const status=target.querySelector('#admin-mau-status'),download=target.querySelector('#admin-mau-download');
  const validate=()=>{const total=config.dialogue.albertoChance+config.dialogue.residualChance;const empty=requiredLineGroups.some(([path])=>!adminMauGet(config,path).length)||Object.values(config.albertoFlow).some(value=>!String(value).trim());const invalidMap=config.mapFlow.length!==11,invalidTiming=Object.values(config.timings).some(value=>!Number.isFinite(Number(value))||Number(value)<0),invalidSecret=!String(config.dialogue.specialGestureMessage||'').trim()||config.timings.albertoDelayMax<config.timings.albertoDelayMin;const valid=total<=1&&!empty&&!invalidMap&&!invalidTiming&&!invalidSecret;download.disabled=!valid;target.querySelector('#admin-mau-probability-summary').textContent=total>1?'La suma de Alberto y anomalías no puede superar el 100 %.':invalidMap?'El mapa narrativo necesita exactamente 11 frases, una por destino.':invalidSecret?'Revisa la animación secreta: la frase es obligatoria y la espera máxima no puede ser menor que la mínima.':`Sobre Alberto ${Math.round(config.dialogue.albertoChance*100)} % · anomalías ${Math.round(config.dialogue.residualChance*100)} % · habituales ${Math.round((1-total)*100)} %. Dentro de las habituales, contextuales ${Math.round(config.dialogue.contextualChance*100)} %.`;status.textContent=valid?'Cambios preparados para descargar.':'Revisa los campos marcados antes de descargar.';return valid};
  const updateOutput=(path,value)=>{const output=target.querySelector(`[data-mau-output="${path}"]`);if(!output)return;output.textContent=path==='minimumPageAge'?`${(value/1000).toFixed(1)} s`:`${Math.round(value*100)} %`};
  target.querySelectorAll('[data-mau-path]').forEach(input=>input.oninput=()=>{let value=input.type==='checkbox'?input.checked:Number(input.value);if(input.dataset.mauScale)value/=Number(input.dataset.mauScale);if(input.dataset.mauMultiplier)value*=Number(input.dataset.mauMultiplier);adminMauSet(config,input.dataset.mauPath,value);if(input.dataset.mauPath==='enabled')input.closest('label').querySelector('span').textContent=value?'Mau activado':'Mau desactivado';updateOutput(input.dataset.mauPath,value);validate()});
  target.querySelectorAll('[data-mau-lines]').forEach(input=>input.oninput=()=>{adminMauSet(config,input.dataset.mauLines,input.value.split('\n').map(line=>line.trim()).filter(Boolean));validate()});
  target.querySelectorAll('[data-mau-text]').forEach(input=>input.oninput=()=>{adminMauSet(config,input.dataset.mauText,input.value.trim());validate()});
  target.querySelectorAll('[data-mau-pose]').forEach(button=>button.onclick=()=>{target.querySelector('#admin-mau-preview-image').src=`../mascot/assets/sprites/mau-${button.dataset.mauPose}.webp`;target.querySelectorAll('[data-mau-pose]').forEach(item=>item.classList.toggle('active',item===button))});
  target.querySelector('#admin-mau-preview-random').onclick=()=>{const pools=[config.dialogue.general,config.dialogue.residual,config.dialogue.alberto].filter(pool=>pool.length),pool=pools[Math.floor(Math.random()*pools.length)],message=pool[Math.floor(Math.random()*pool.length)];target.querySelector('#admin-mau-preview-message').textContent=message};
  target.querySelector('#admin-mau-restore').onclick=async()=>{if(!await adminConfirm({title:'Restaurar configuración publicada',message:'Se descartarán los cambios realizados en este editor. El archivo publicado no se modificará.',confirmLabel:'Restaurar valores'}))return;adminMauDraft=cloneAdminMauConfig(adminMauPublishedConfig);renderAdminMauEditor()};
  download.onclick=()=>{if(!validate())return;const blob=new Blob([adminMauSerialize(config)],{type:'text/javascript;charset=utf-8'}),url=URL.createObjectURL(blob),anchor=document.createElement('a');anchor.href=url;anchor.download='mau-config.js';document.body.appendChild(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);status.textContent='Archivo descargado. Sustituye mascot/mau-config.js y publica el proyecto.'};
  validate();status.textContent='Configuración publicada cargada.';
};
...
const renderAdminEditorLegacy=(profile,state)=>{
  ...
  document.querySelector('#admin-reset-progress').onclick=async()=>{
    ...
    try{
      ...
      if(error)throw error;
      if(data?.deviceProgressReset!==true)throw new Error('El servidor no ha confirmado el reinicio de las evidencias del dispositivo.');
...
const renderAdminEditor=(profile,state,initialTab='summary')=>{
  ...
  simulationsPanel.append(earlyAccessSettings);
  ...
  const albertoSettings=document.createElement('section'),expedientCompleted=Boolean(current.completed&&selected.has('KTB-014')),albertoPhysicalRead=Boolean(current.albertoPhysicalLetterRead||current.albertoMessageRead),albertoResponded=current.albertoResponseAccepted===true,albertoAuthorized=current.albertoEnvelopeAuthorized===true;
  albertoSettings.className='admin-message-settings admin-alberto-settings';
  albertoSettings.innerHTML=`<p class="system-line">COMUNICACIÓN FINAL</p><h4>Carta de Alberto</h4><p>${!expedientCompleted?'Estas acciones estarán disponibles cuando el expediente esté completado.':albertoResponded?'La carta física está leída y la decisión final ya está registrada. Puedes reabrir únicamente la respuesta o reiniciar toda la experiencia.':albertoPhysicalRead?'La carta física ya está leída y permanece pendiente de respuesta.':albertoAuthorized?'La apertura del sobre está autorizada. El destinatario todavía no ha confirmado la lectura física.':'La carta está pendiente de autorización y apertura.'}</p><div class="admin-alberto-actions"><button type="button" id="admin-reopen-alberto-response" ${expedientCompleted&&albertoResponded?'':'disabled'}>Reabrir respuesta final</button><button type="button" id="admin-reset-alberto" ${expedientCompleted?'':'disabled'}>Reiniciar experiencia de la carta</button></div><span id="admin-alberto-status" role="status"></span>`;
  ...
  alt00Settings.className='admin-message-settings admin-alt00-settings';
  alt00Settings.innerHTML=`<div class="admin-alt00-heading"><div><p class="system-line">CIERRE CINEMATOGRÁFICO · PROTOCOLO OMEGA</p><h4>Final y registro ALT-00</h4></div><span class="admin-alt00-state ${alt00Completed?'completed':alt00Discovered?'discovered':'hidden'}">${alt00Completed?'COMPLETADO':alt00Discovered?`DESCUBIERTO · PÁGINA ${Math.max(1,Math.min(10,Number(current.alt00LastPage)||1))} DE 10`:'NO DESCUBIERTO'}</span></div><p>La secuencia aplicada detecta y presenta ALT-00 después de «Buen viaje». Conservamos el cierre anterior como referencia. Ninguna prueba modifica el expediente.</p><div class="admin-alt00-preview-actions"><button type="button" id="admin-play-finale"><span aria-hidden="true">▶</span> Simular cierre anterior</button><button type="button" id="admin-preview-alt00-response">Simular secuencia aplicada</button></div><small>VISTAS DE PRUEBA · SIN ESCRITURAS EN SUPABASE</small>`;
  const recoveredDeviceSettings=document.createElement('section');
  recoveredDeviceSettings.className='admin-message-settings admin-recovered-device-settings';
  recoveredDeviceSettings.innerHTML=`<p class="system-line">PROTOTIPO AISLADO · AR-06</p><h4>Dispositivo recuperado</h4><p>Abre la reconstrucción independiente del teléfono SM-G991B y sus aplicaciones recuperadas. Esta prueba no modifica el expediente, el progreso ni Supabase.</p><button type="button" id="admin-open-recovered-device">Abrir dispositivo recuperado ↗</button><small>ACCESO DE ADMINISTRACIÓN · SE ABRE EN UNA PESTAÑA NUEVA</small>`;
  simulationsPanel.append(recoveredDeviceSettings,alt00Settings);
...
function openComicViewer(page){
  ...
  paper.classList.remove('is-ac-info-paper');
  body.classList.remove('is-ac-info');
  paper.style.width='';
...
// Puentes mínimos hacia KIZUNA Microevents. La lógica documental permanece aquí;
...
setTimeout(async()=>{
  const module=await getMicroevents();if(!module)return;
  const originalMarkHandler=mark.onclick;
  ...
  const {nav,view}=module.createAdmin();
  document.querySelector('.admin-sidebar')?.appendChild(nav);
  document.querySelector('.admin-views')?.appendChild(view);
  const bar=document.createElement('header');bar.className='admin-section-bar';bar.innerHTML='<span>INTERACCIONES</span><strong>Microeventos del expediente</strong>';view.prepend(bar);
  document.querySelector('.admin-sidebar')?.addEventListener('click',event=>{
    const button=event.target.closest('button[data-admin-view]');if(button&&button!==nav)view.hidden=true;
  });
  nav.onclick=async()=>{
    document.querySelectorAll('.admin-sidebar button').forEach(button=>button.classList.toggle('active',button===nav));
    document.querySelectorAll('.admin-views>.admin-view').forEach(panel=>panel.hidden=panel!==view);

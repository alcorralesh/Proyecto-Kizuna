...
const renderAdminMauEditor=()=>{
  ...
  target.querySelector('#admin-mau-preview-random').onclick=()=>{const pools=[config.dialogue.general,config.dialogue.residual,config.dialogue.alberto,config.interaction.awake].filter(pool=>pool.length),pool=pools[Math.floor(Math.random()*pools.length)],message=pool[Math.floor(Math.random()*pool.length)];target.querySelector('#admin-mau-preview-message').textContent=message};

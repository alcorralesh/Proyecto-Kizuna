...
const renderAdminMauEditor=()=>{
  ...
  const config=adminMauDraft,dialogueGroups=[['general','Frases generales'],['residual','Frases residuales · easter egg'],['alberto','Frases sobre Alberto'],['interaction.awake','Respuestas al saludar'],['interaction.sleeping','Respuestas mientras duerme'],['mapFlow','Mapa narrativo · 11 destinos']];
  target.innerHTML=`<div class="admin-mau-layout"><section class="admin-mau-controls"><header><div><p class="system-line">COMPORTAMIENTO GLOBAL</p><h2>Apariciones y frecuencia</h2></div><label class="admin-mau-enabled"><input type="checkbox" data-mau-path="enabled" ${config.enabled?'checked':''}><i></i><span>${config.enabled?'Mau activado':'Mau desactivado'}</span></label></header><div class="admin-mau-field-grid"><label>Primera aparición <output data-mau-output="triggerProgress">${Math.round(config.triggerProgress*100)} % de página</output><input type="range" min="5" max="90" step="5" value="${Math.round(config.triggerProgress*100)}" data-mau-path="triggerProgress" data-mau-scale="100"></label><label>Espera mínima <output data-mau-output="minimumPageAge">${(config.minimumPageAge/1000).toFixed(1)} s</output><input type="range" min="0" max="30" step="1" value="${config.minimumPageAge/1000}" data-mau-path="minimumPageAge" data-mau-scale="0.001"></label><label>Apariciones despierto<input type="number" min="1" max="5" step="1" value="${config.maxAwakeAppearances}" data-mau-path="maxAwakeAppearances"></label><label>Repetición mínima<input type="number" min="5" max="180" step="5" value="${config.repeatCooldown/1000}" data-mau-path="repeatCooldown" data-mau-multiplier="1000"><small>Segundos entre apariciones</small></label><label>Postura especial <output data-mau-output="specialGestureChance">${Math.round(config.specialGestureChance*100)} %</output><input type="range" min="0" max="100" step="5" value="${Math.round(config.specialGestureChance*100)}" data-mau-path="specialGestureChance" data-mau-scale="100"></label><label>Duración de la burbuja<input type="number" min="2" max="30" step=".5" value="${config.timings.message/1000}" data-mau-path="timings.message" data-mau-multiplier="1000"><small>Segundos visibles</small></label></div></section><aside class="admin-mau-preview"><p class="system-line">VISTA PREVIA</p><div class="admin-mau-preview-stage"><div class="admin-mau-preview-bubble" id="admin-mau-preview-message">${adminEditorEscape(config.dialogue.general[0])}</div><img id="admin-mau-preview-image" src="../mascot/assets/sprites/mau-guide.webp" alt="Previsualización de Mau"></div><div class="admin-mau-preview-actions"><button type="button" data-mau-pose="peek">Asomado</button><button type="button" data-mau-pose="guide">Normal</button><button type="button" data-mau-pose="leave">Marchándose</button><button type="button" data-mau-pose="sleep">Dormido</button><button type="button" id="admin-mau-preview-random">Probar frase aleatoria</button></div></aside><section class="admin-mau-probabilities"><p class="system-line">SELECCIÓN AUTOMÁTICA</p><h2>Probabilidad de cada tipo de frase</h2><div class="admin-mau-field-grid"><label>Sobre Alberto <output data-mau-output="dialogue.albertoChance">${Math.round(config.dialogue.albertoChance*100)} %</output><input type="range" min="0" max="100" step="5" value="${Math.round(config.dialogue.albertoChance*100)}" data-mau-path="dialogue.albertoChance" data-mau-scale="100"></label><label>Residual / easter egg <output data-mau-output="dialogue.residualChance">${Math.round(config.dialogue.residualChance*100)} %</output><input type="range" min="0" max="100" step="5" value="${Math.round(config.dialogue.residualChance*100)}" data-mau-path="dialogue.residualChance" data-mau-scale="100"></label><label>Contextual dentro del resto <output data-mau-output="dialogue.contextualChance">${Math.round(config.dialogue.contextualChance*100)} %</output><input type="range" min="0" max="100" step="5" value="${Math.round(config.dialogue.contextualChance*100)}" data-mau-path="dialogue.contextualChance" data-mau-scale="100"></label></div><p id="admin-mau-probability-summary"></p></section><section class="admin-mau-dialogues"><p class="system-line">GUION DE MAU</p><h2>Frases disponibles</h2><p>Escribe una frase por línea. Las líneas vacías se descartan al generar el archivo.</p><div class="admin-mau-dialogue-grid">${dialogueGroups.map(([path,title])=>`<label>${title}<textarea rows="7" data-mau-lines="${path}">${adminEditorEscape(adminMauLines(adminMauGet(config,path)))}</textarea></label>`).join('')}</div><details><summary>Frases contextuales por sección</summary><div class="admin-mau-dialogue-grid">${Object.entries(config.dialogue.contextual).map(([section,lines])=>`<label>${section}<textarea rows="5" data-mau-lines="dialogue.contextual.${section}">${adminEditorEscape(adminMauLines(lines))}</textarea></label>`).join('')}</div></details></section><section class="admin-mau-flow"><p class="system-line">CARTA DE ALBERTO</p><h2>Mensajes durante el flujo final</h2><div>${Object.entries(adminMauFlowLabels).map(([key,label])=>`<label>${label}<textarea rows="3" data-mau-text="albertoFlow.${key}">${adminEditorEscape(config.albertoFlow[key])}</textarea></label>`).join('')}</div></section><details class="admin-mau-advanced"><summary>Tiempos y animaciones avanzadas</summary><div class="admin-mau-timing-grid">${Object.entries(config.timings).filter(([key])=>key!=='message').map(([key,value])=>`<label>${adminMauTimingLabels[key]||key}<input type="number" min="50" max="180000" step="50" value="${value}" data-mau-path="timings.${key}"><small>Milisegundos</small></label>`).join('')}</div></details><footer class="admin-mau-export"><div><p class="system-line">ARCHIVO DE CONFIGURACIÓN</p><strong>mascot/mau-config.js</strong><span id="admin-mau-status" role="status">Sin cambios.</span></div><button type="button" id="admin-mau-restore">Restaurar archivo publicado</button><button type="button" id="admin-mau-download">Descargar configuración</button></footer></div>`;
  const status=target.querySelector('#admin-mau-status'),download=target.querySelector('#admin-mau-download');
  const validate=()=>{const total=config.dialogue.albertoChance+config.dialogue.residualChance;const empty=dialogueGroups.some(([path])=>!adminMauGet(config,path).length)||Object.values(config.albertoFlow).some(value=>!String(value).trim());const invalidMap=config.mapFlow.length!==11,invalidTiming=Object.values(config.timings).some(value=>!Number.isFinite(Number(value))||Number(value)<0);const valid=total<=1&&!empty&&!invalidMap&&!invalidTiming;download.disabled=!valid;target.querySelector('#admin-mau-probability-summary').textContent=total>1?'La suma de Alberto y residual no puede superar el 100 %.':invalidMap?'El mapa narrativo necesita exactamente 11 frases, una por destino.':`Alberto ${Math.round(config.dialogue.albertoChance*100)} % · residual ${Math.round(config.dialogue.residualChance*100)} % · resto ${Math.round((1-total)*100)} %.`;status.textContent=valid?'Cambios preparados para descargar.':'Revisa los campos marcados antes de descargar.';return valid};

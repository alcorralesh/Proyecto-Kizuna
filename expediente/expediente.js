...
const renderAdminEditor=(profile,state)=>{
  ...
  document.querySelector('#admin-view-all-activity').onclick=()=>activateTab('activity');
  document.querySelector('.admin-open-identity').onclick=()=>{activateTab('documents');const details=document.querySelector('.admin-identity-details');details.open=true;details.scrollIntoView({behavior:'smooth',block:'center'})};
  ...
  const saveProgress=async()=>{const status=document.querySelector('#admin-save-status'),button=document.querySelector('#admin-save-progress');const records=[...editor.querySelectorAll('input[name="records"]:checked')].map(input=>input.value),closing=records.includes('KTB-014');const nextState={...current,read:records,completed:closing?current.completed:false,finalAlertShown:closing?current.finalAlertShown:false,finalFileSeen:closing?current.finalFileSeen:false};status.textContent='Guardando…';button.disabled=true;const {error}=await supabaseClient.from('expedient_progress').upsert({user_id:profile.id,state:nextState,updated_at:new Date().toISOString()});if(error){status.textContent='No se pudieron guardar los cambios.';console.error(error);button.disabled=false;return}renderAdminEditor(profile,nextState)};
...
const renderAdminActivity=async userId=>{
  const full=document.querySelector('#admin-activity-log'),preview=document.querySelector('#admin-activity-preview'),last=document.querySelector('#admin-last-activity');if(!full||!preview)return;
  try{const {data,error}=await supabaseClient.from('expedient_activity_log').select('event_type,document_id,details,created_at').eq('user_id',userId).order('created_at',{ascending:false}).limit(50);if(error)throw error;const items=data||[];last.textContent=items[0]?new Date(items[0].created_at).toLocaleDateString('es-ES'):'Sin actividad';
    ...
    if(!items.length){full.innerHTML='<p class="system-line">REGISTRO DE ACTIVIDAD</p><h3>Actividad del expediente</h3><p>Aún no hay actividad registrada para este destinatario.</p>';preview.innerHTML='<p>Sin actividad registrada.</p>';return}
    full.innerHTML=`<p class="system-line">REGISTRO DE ACTIVIDAD</p><h3>Actividad del expediente</h3><ol class="admin-activity-list">${items.map(row).join('')}</ol>`;preview.innerHTML=`<ol class="admin-activity-list compact">${items.slice(0,3).map(row).join('')}</ol>`;
  ...
  }catch(error){console.error(error);last.textContent='No disponible';full.innerHTML='<p>No se ha podido recuperar la actividad.</p>';preview.innerHTML='<p>Actividad no disponible.</p>'}
};

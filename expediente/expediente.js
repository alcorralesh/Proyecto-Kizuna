...
const readerTools=document.querySelector('.reader-tools'),readerZoom=document.querySelector('#reader-zoom'),readerStatus=document.querySelector('#reader-status'),readerCode=document.querySelector('#reader-code'),readerContent=document.querySelector('.reader-content');
...
const syncReaderChrome=()=>{
  ...
  const actionable=mark.style.display!=='none';
  const alreadyRead=Boolean(active&&read().includes(active));
  readerStatus.classList.toggle('is-confirmed',actionable&&alreadyRead);
  readerStatus.textContent=!actionable?'CONSULTA INTERNA':alreadyRead?'LECTURA CONFIRMADA':'PENDIENTE DE CONFIRMACIÓN';
  if(actionable&&alreadyRead)mark.textContent='Volver al expediente';
};
...
viewer.addEventListener('close',()=>{if(viewer.classList.contains('is-fallback-fullscreen')){viewer.classList.remove('is-fallback-fullscreen');document.body.style.overflow=readerPreviousBodyOverflow}if(document.fullscreenElement===viewer)document.exitFullscreen();setReaderFullscreenState(false)});
...
function openFolderFile(folderId,fileId){const file=folders[folderId].files.find(item=>item.id===fileId);active=fileId;next.style.display='none';mark.style.display='inline-block';mark.textContent=read().includes(fileId)?'Volver al expediente':'Confirmar lectura';document.querySelector('#doc-type').textContent=`${folderId} / DOCUMENTO INTERNO`;document.querySelector('#doc-title').textContent=file.title;document.querySelector('#doc-body').innerHTML=`<img style="display:block;width:100%;height:auto" src="../assets/documents/${folderId}/${file.src}" alt="${file.title}">`}
function openFolderFile(folderId,fileId){const file=folders[folderId].files.find(item=>item.id===fileId);if(file.mosaic==='tickets'){openTicketMosaic();return}active=fileId;next.style.display='none';mark.style.display='inline-block';mark.textContent=read().includes(fileId)?'Volver al expediente':'Confirmar lectura';document.querySelector('#doc-type').textContent=`${folderId} / DOCUMENTO INTERNO`;document.querySelector('#doc-title').textContent=file.title;document.querySelector('#doc-body').innerHTML=`<img style="display:block;width:100%;height:auto" src="../assets/documents/${folderId}/${file.src}" alt="${file.title}">`}
